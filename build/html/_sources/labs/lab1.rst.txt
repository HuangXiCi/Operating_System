...
本课程直接使用预编译的 RISC-V GNU 工具链和 QEMU。下载、解压后即可使用，**不要求从源码编译**。
完整编译工具链不仅耗时较长，还会占用大量磁盘空间，本身也不是本次操作系统实验的目标。

以下操作都在终端中完成。先安装 Git、Make、调试器和解压工具等基础软件：

.. code-block:: bash

   sudo apt update
   sudo apt install git build-essential gdb-multiarch xz-utils nano
...
在 Linux 主目录创建课程工作目录，并获取 MIT 官方 xv6-riscv 源码：
...
如果课程另行提供了指定仓库或分支，应以课程通知为准，不要同时在多个仓库中修改代码。
...
``git status`` 可以显示当前分支和工作区是否有改动；最后一条命令输出当前提交的短哈希值。
记录提交值有助于在出现差异时确认大家使用的源码是否一致。
...
可以在 **Linux shell** 中执行下面的命令进行初步观察：
...
这是两个依次完成的按键动作，不是同时按下三个键。退出后会重新回到 Ubuntu shell。
...
* `RISC-V GNU Toolchain 预编译版本 <https://github.com/riscv-collab/riscv-gnu-toolchain/releases>`_
* `xPack QEMU RISC-V 安装文档 <https://xpack-dev-tools.github.io/qemu-riscv-xpack/docs/install/>`_
